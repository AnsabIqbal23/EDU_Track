...
export default function BorrowBook() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const response = await fetch(`http://localhost:8081/api/library/borrow?bookId=${data.bookId}&studentId=${data.studentId}`, {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                showToast("success", responseData.message || "Book borrowed successfully!");
            } else {
                showToast("error", responseData.message || "Something went wrong.");
            }
        } catch (error) {
            showToast("error", "An error occurred while borrowing the book.");
        }
                ...
                <div className="grid grid-cols-[auto_1fr]">
                    ...
                    <main className="p-8">
                        <header className="mb-8">
                            <h1 className="text-3xl font-bold text-white mb-2">Borrow Book</h1>
                            <p className="text-gray-400">Enter the book and student details to borrow a book.</p>
                        </header>

                        <Card className="bg-gray-800/50 border-gray-700 backdrop-blur-sm">
                            <CardHeader className="p-6">
                                <h2 className="text-2xl font-bold text-white">Borrow Book Information</h2>
                            </CardHeader>
